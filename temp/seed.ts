...
const updateIntoTable = 
    ...
    const sql: string = `
        UPDATE ${tableName} SET ${fields} WHERE ${where}
    `
...
const seed = async (): Promise<void> => {
    try {
        ...
        const compliteCreateTables = Promise.all(
            tables.map(async (table) => {
                return await db.execute(`
                    CREATE TABLE ${table.name} ( ${table.fields.join(', ')} )
                `)
            })
        )   
                        ...
                        userListings.map(async (listingId): Promise<number> => {
                            ...
                            if (listing) {
                                ...
                                const values: any[] = [
                                    ...
                                    listing.admin,
                                    listing.city,
                                    JSON.stringify(listing.bookings),
                                    JSON.stringify(listing.bookingsIndex),
                                    listing.price,
                                    listing.numOfGuests
                                ]
                                const newListing = await insertIntoTable(db, 'listings', fields, values)
                                return newListing.insertId
                            }
